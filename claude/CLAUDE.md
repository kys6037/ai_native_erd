...
### WebSocket 협업 동시성 설계
```
클라이언트 A ──── Yjs update (binary) ────▶ CollaborationHandler
클라이언트 B ──── Yjs update (binary) ────▶ (같은 projectId room)
                                              │
                                              ▼
                                     broadcast to all except sender
                                     (ConcurrentHashMap<projectId, Set<WsContext>>)
```
- **동시성 제어**: `ConcurrentHashMap`으로 room 관리, Javalin WS는 이벤트 기반(단일 스레드 per connection)
- **충돌 해결**: Yjs CRDT 알고리즘이 클라이언트 측에서 충돌 자동 병합 (서버는 relay만 담당)
- **연결 해제**: `onClose`/`onError`에서 room에서 제거, 빈 room은 자동 cleanup
- **인증**: WebSocket upgrade 시 `?token=` query param으로 JWT 검증 (헤더 방식은 브라우저 WS API 미지원)
